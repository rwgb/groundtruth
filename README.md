...
## What it produces — the verdict card

Written to `.claude/groundtruth/<session>.md` each turn (and surfaced live via the status badge):
...
3. **`/groundtruth-setup`** — a one-shot check that detects what's configured and hands you the exact
   action for anything still missing (a plugin can't set the main `statusLine` or env vars itself, so
   those few steps are guided — see [Configure](#configure) below).
4. **`/groundtruth-rules approve-all`** — on session start Groundtruth already read your rule docs
   (`CLAUDE.md`, `SCHEMA.md`, your skills, …) and **proposed** deterministic rules. This arms the *clean*
   ones (the ones that match zero existing code). **Nothing is enforced until you run this** — it's a
   permission gate, not automatic.
5. **Just work.** Every turn now gets a verdict — **warn by default, so it never interrupts you.** See it
   via the status badge, or run **`/groundtruth`** for the full card. Once you trust the precision on your
   real sessions, opt into blocking with `/groundtruth-block on`.
